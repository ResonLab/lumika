...
import { cheminBase } from '../contexte.ts'
import { appliquerMigrations } from './migrations.ts'

/**
 * La base SQLite de Lumika.
 *
 ...
 *
 * **Le schéma arrive en paramètre, il n'est pas importé ici.** Il vivait dans
 * un `import ... from './schema.sql?raw'`, une syntaxe que seul l'empaqueteur
 * comprend : la couche base devenait alors impossible à charger depuis Node, et
 * donc impossible à éprouver sans lancer une fenêtre. C'est la fenêtre qui lit
 * le fichier et le passe — même raison que pour Electron, qui n'entre pas non
 * plus ici.
 */
let db: DatabaseSync | null = null

export function ouvrirBaseDeDonnees(schema: string): DatabaseSync {
  if (db) return db
